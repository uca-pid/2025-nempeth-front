          ...
          generateLabels: function(chart: any) {
            ...
            if (data.labels.length && data.datasets.length) {
              return data.labels.map((label: string, i: number) => {
                ...
                // Extraer solo el nombre de la categoría para la leyenda
      ...
      tooltip: {
        callbacks: {
          label: function(context: any) {
            // Extraer solo el nombre de la categoría de la etiqueta completa
            const fullLabel = context.label
            const categoryName = fullLabel.split(':')[0]
            const total = context.dataset.data.reduce((a: number, b: number) => a + b, 0)
            const percentage = ((context.parsed / total) * 100).toFixed(1)
            return `${categoryName}: $${context.parsed.toLocaleString()} (${percentage}%)`
          },
          title: function() {
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          ...
          title: function() {
            return 'Ganancias por categoría'
          }
        }
      }
    },
